...
// Fills the official BC RTB-1 Residential Tenancy Agreement from HomeSuite lease
// data. Field IDs verified against the 2023/06 form (122 fields). Text fields
// get string values; checkboxes are checked when the mapped key is present.

export interface RTB1Party {
  last: string
  first: string
}

export interface RTB1Address {
  unit?: string
  street?: string
  city?: string
  province?: string
...
export interface RTB1Data {
  landlords: RTB1Party[]
  tenants: RTB1Party[]
  tenant_phone?: string
  tenant_email?: string
  rental_unit: RTB1Address
  service_address?: RTB1Address
...
// HomeSuite included-utility names -> RTB-1 checkbox field ids.
...
/** Returns the filled RTB-1 as PDF bytes. */
export async function fillRTB1(template: Uint8Array, data: RTB1Data): Promise<Uint8Array> {
  ...
  // --- Address for service (defaults to rental unit) ---
  ...
  if (ttype === "periodic") {
    ...
    if (ptype === "monthly") {
      checks.push(
        "A and continues on a monthtomonth basis until ended in accordance with the Act",
      )
    } else {
      checks.push(
  ...
  // --- Page 6: printed names in signature blocks (signature lines stay blank) ---
  if (data.landlords[0]) {
  ...
  for (const [id, val] of Object.entries(text)) {
    ...
    try {
      ...
      // Field id not present in this form version — skip silently.
    }
  }
  // Apply checkboxes
